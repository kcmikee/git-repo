...
import { sortLanguages } from "../utils/sort";
...
function Search() {
  ...
  const handleFilter = (filter) => {
    ...
    if (filter === "clear" || filter === "Any") {
      ...
      let filteredResults = data?.items?.filter((data) => data.language === filter);
      setFilteredResults({
  ...
  const handlePagination = (direction) => {
    let offset = page * 31;
    let results = filteredResults ? filteredResults?.totalCount : data?.totalCount || 0;
  ...
  return (
    <Container id="Search">
      ...
      <ResultsWrapper>
        {/* <Filters data={data} handleFilter={handleFilter} setFilterActive={setFilterActive} /> */}
        <List>
          <Dropdowns>
            <Dropdown
          ...
          <TotalResults>
            {filteredResults ? filteredResults?.totalCount : data?.totalCount || 0} results
          </TotalResults>
          {loading ? (
            <Loading>
              <Loader size={70} color={"#19487b"} />
            </Loading>
          ) : data ? (
            <>
              <SearchResults filteredResults={filteredResults} fetchData={fetchData} data={data} />
              <Pagination>
